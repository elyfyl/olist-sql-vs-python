...
Kullandığım veri seti, Brezilya'daki birden fazla pazaryerinde gerçekleşen 100 bin siparişe ait gerçek ticari verilerden oluşmaktadır. Olist Store tarafından sağlanan bu veriler; sipariş durumu, fiyat, müşteri konumu ve ürün özellikleri gibi birçok boyutta analiz yapmaya olanak tanır. 
...
### SQL Yaklaşımı
SQL kullanarak veritabanına sorduğumuz sorunun mantığı şu şekildeydi:
* **JOIN (Birleştirme):** Tarih bilgisi `orders`, fiyat bilgisi `order_items` tablosunda olduğu için `order_id` üzerinden birleştirme yaptık.
...
İkinci problemimizde, hangi kategoriden sayıca daha çok ürün satıldığını (satış adedine göre sipariş hacmini) analiz ettik. Müşteri tercihlerini anlamak için kritik bir metrik olan "satış adedini" bulmak için şu adımları izledik:

### Mantık ve İşlem Adımları
Hem SQL hem de Python'da aynı kurguyu uyguladık:
1. **Birleştirme:** `order_items` (satış detayları) tablosu ile `products` (ürün bilgileri) tablosunu `product_id` üzerinden yan yana getirdik.
